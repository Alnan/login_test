...
from students.permission_list import perm_dic
...
def perm_check(*args,**kwargs):
    ...
    resolve_url_obj = resolve(request.path)#通过resolve（）方法，可获得url_name  ，resolve_url_obj是个集合
    # print("resolve_url_obj:", resolve_url_obj.args[0])
    current_url_name = resolve_url_obj.url_name  # 当前url的url_name
    print('---perm:',request.user,request.user.is_authenticated,current_url_name,request.method)
    #match_flag = False
    match_results = [None,]
    match_key = None
    ...
    if all(match_results): # true or false
        ...
        if request.user.has_perm(perm_obj): # obj.has_perm(crm.crm_table_list): 判断是否有权限
            print('当前用户有此权限')
            return True
        else:
            print('当前用户没有该权限')
